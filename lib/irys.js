...
async function fetchContentFromIrys(cid) {
  ...
  try {
    ...
    console.log("there was an error fetching from irys", error);
    return null;
  }
}
...
async function getThisUserWritings(authorAddress) {
  ...
  //       ankyverseDay: result?.tags[4]?.value || undefined,
  //     };
  //   })
  // );
  ...
  let allUserWritings = await Promise.all(
    results.map(async (result) => {
      ...
      const thisText = await content.text();
      const calculatedDay =
        Math.floor((result.timestamp / 1000 - startTimestamp) / (24 * 3600)) +
        1;
      console.log("the calculated day is: ", calculatedDay);
      const storedDay = parseInt(
        result.tags.find((tag) => tag.name === "day").value,
        10
      );
      console.log("the stored day is :", storedDay);
      return {
        cid: result.id,
        timestamp: result.timestamp,
        text: thisText,
        ...
        storedDay: storedDay,
        color: calculatedDay == storedDay ? "green" : "purple", // Determine color based on day match
      };
    })
  );
  console.log("all the user writings are", allUserWritings);
  return allUserWritings.filter((x) => x.storedDay > 0);
}
